...
수신자가 메시지를 받는 방식은 여러 가지가 있으나, 본 설계에서는 **웹소켓(WebSocket)**을 채택합니다.
...
발신자 측은 TCP 핸드셰이크 횟수를 줄이기 위해 keep-alive 헤더를 포함한 HTTP 프로토콜을 사용합니다.
...
![img.png](img.png)
모든 서비스는 데이터 저장을 위해 **키-값 저장소(KV Store)**를 사용합니다.
...
**Message ID**: 메시지 ID는 고유해야 하며 시간 순으로 정렬 가능해야 합니다.
